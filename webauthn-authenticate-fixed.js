...
async function handler(event, context) {
    ...
    try {
        ...
        const { action } = body;
        logging_js_1.logger.info('🔐 WebAuthn handler called', { action, hasEmail: !!body.email });
        if (action === 'start') {
...
async function startAuthentication(email, origin) {
    ...
    let userId;
    logging_js_1.logger.info('🔐 WebAuthn startAuthentication called', { email });
    if (email) {
        // Buscar credenciais do usuário específico
        let user = await prisma.user.findUnique({
            where: { email }
        });
        logging_js_1.logger.info('🔐 User lookup result', { email, userFound: !!user, userId: user?.id });
        if (!user) {
            logging_js_1.logger.info('🔐 User not found in database, checking if exists in Cognito', { email });
            ...
            logging_js_1.logger.info('🔐 User not in database - no WebAuthn credentials possible', { email });
        ...
        logging_js_1.logger.info('🔐 WebAuthn credentials lookup', {
            userId: user.id,
        ...
        if (webauthnCredentials.length === 0) {
            logging_js_1.logger.info('🔐 No WebAuthn credentials found - user can proceed with normal login', { email, userId: user.id });
        ...
        logging_js_1.logger.info('🔐 WebAuthn credentials found', {
            email,
            userId: user.id,
    ...
    if (!userId) {
        return (0, response_js_1.error)('User ID required for WebAuthn', 400, undefined, origin);
    }
    // Gerar challenge
