...
function generateFunction(h: HandlerConfig): string {
  ...
  // Use esbuild for bundling - much faster and smaller bundles
  const metadata = `
    Metadata:
      BuildMethod: esbuild
      BuildProperties:
        ...
        Sourcemap: false
        EntryPoints:
          - src/handlers/${h.path}/${h.handler}.ts
        External:
          - '@prisma/client'
          - '.prisma/client'`;
  ...
  ${name}Function:
    ...
    Properties:
      ...
      CodeUri: backend/
      Handler: ${h.handler}.handler${extraProps}${events}${metadata}`;
}
...
Description: |
  ...
  Using esbuild for fast bundling (2026-02-04)
