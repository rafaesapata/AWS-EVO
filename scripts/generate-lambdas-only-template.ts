#!/usr/bin/env npx tsx
/**
 * EVO Platform - Lambdas-Only SAM Template Generator
 * 
 * Generates SAM template for Lambda functions ONLY
 * Uses existing VPC, RDS, Cognito infrastructure via parameters
 * All functions use ARM64 + esbuild for fast builds
 * 
 * Usage: npx tsx scripts/generate-lambdas-only-template.ts
 */
...
Description: |
  EVO Platform - Lambda Functions Only (${HANDLERS.length} total)
...
`;
  ...
  const categories = new Map<string, HandlerConfig[]>();
  for (const h of HANDLERS) {
    const cat = h.path.toUpperCase();
  ...
  let functions = '';
  for (const [cat, handlers] of categories) {
    functions += `
  # ==========================================================================
  # ${cat} FUNCTIONS (${handlers.length})
  # ==========================================================================`;
    for (const h of handlers) {
      functions += generateFunction(h);
    }
  }
...
`;
  ...
  return header + layer + iamRole + httpApi + functions + outputs;
}

// Main
const template = generateTemplate();
const outputPath = path.join(process.cwd(), 'sam', 'production-lambdas-only.yaml');
fs.writeFileSync(outputPath, template);
console.log('Generated SAM template with ' + HANDLERS.length + ' Lambda functions');
