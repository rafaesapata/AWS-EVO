...
async function handler(event, _context) {
    ...
    try {
        ...
        logging_js_1.logger.info(`License validation for org ${organizationId}, user ${userId}`);
        const prisma = (0, database_js_1.getPrismaClient)();
        // Check if customer_id is being configured
        const body = event.body ? JSON.parse(event.body) : {};
        logging_js_1.logger.info(`Request body: ${JSON.stringify(body)}, has customer_id: ${!!body.customer_id}`);
        if (body.customer_id) {
            // Only admins can configure license
            if (!(0, auth_js_1.isAdmin)(user)) {
                return (0, response_js_1.error)('Only administrators can configure license settings', 403, undefined, origin);
            }
            // Validate customer_id format (UUID)
            const uuidRegex = /^[a-f0-9]{8}-[a-f0-9]{4}-[a-f0-9]{4}-[a-f0-9]{4}-[a-f0-9]{12}$/i;
            if (!uuidRegex.test(body.customer_id)) {
                return (0, response_js_1.error)('Invalid customer_id format. Must be a valid UUID.', 400, undefined, origin);
            }
            logging_js_1.logger.info(`Configuring license for org ${organizationId} with customer_id ${body.customer_id}`);
            // Upsert config
            ...
            if (!syncResult.success) {
                logging_js_1.logger.error(`License sync failed: ${syncResult.errors.join(', ')}`);
                return (0, response_js_1.error)('Failed to validate license with external system', 400, undefined, origin);
            }
            logging_js_1.logger.info(`License sync successful: ${syncResult.licensesSynced} licenses synced`);
        }
        ...
        logging_js_1.logger.info(`License summary for org ${organizationId}: hasLicense=${summary.hasLicense}, customerId=${summary.customerId}, licenses=${summary.licenses?.length || 0}`);
        if (!summary.hasLicense) {
            ...
            logging_js_1.logger.info(`License config for org ${organizationId}: ${config ? `customer_id=${config.customer_id}, sync_status=${config.sync_status}` : 'not found'}`);
        ...
        logging_js_1.logger.info(`User ${userId} seat check: has_seat=${!!userSeat}`);
        // AUTO-ASSIGNMENT: If user doesn't have a seat, try to assign one automatically
        if (!userSeat) {
            logging_js_1.logger.info(`Attempting auto-assign for user ${userId}`);
            ...
            if (availableLicense) {
                ...
                logging_js_1.logger.info(`Found license ${availableLicense.id}: max_users=${availableLicense.max_users}, used=${usedSeats}, available=${availableSeats}`);
                if (availableSeats > 0) {
                    logging_js_1.logger.info(`Auto-assigning seat to user ${userId} on license ${availableLicense.id}`);
                    const assignResult = await (0, license_service_js_1.assignSeat)(availableLicense.id, userId, undefined);
                    if (assignResult.success) {
                        logging_js_1.logger.info(`Seat auto-assigned successfully to user ${userId}`);
                        ...
                        logging_js_1.logger.warn(`Failed to auto-assign seat: ${assignResult.error}`);
                    }
                }
                else {
                    logging_js_1.logger.warn(`No available seats for user ${userId} in org ${organizationId}`);
                }
            }
        }
        // Get usage stats
        ...
        const primaryLicense = summary.licenses.find((l) => !l.isExpired) || summary.licenses[0];
        // Generate alerts
        const alerts = [];
        if (primaryLicense) {
        ...
        const hasValid = await (0, license_service_js_1.hasValidLicense)(organizationId);
        ...
        return (0, response_js_1.success)({
            ...
            licenses: summary.licenses.map((l) => ({
        ...
        logging_js_1.logger.error('Validate license error:', err);
        // Don't expose internal error details to frontend
        return (0, response_js_1.error)('License validation failed. Please try again.', 500, undefined, origin);
    }
}
//# sourceMappingURL=validate-license.js.map