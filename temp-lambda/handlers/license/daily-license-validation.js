...
async function handler(event, context) {
    ...
    const origin = !isScheduled ? (0, middleware_js_1.getOrigin)(event) : undefined;
    const httpMethod = !isScheduled ? (event.httpMethod || event.requestContext?.http?.method) : undefined;
    ...
    if (!isScheduled) {
        try {
            ...
            filterOrgId = (0, auth_js_1.getOrganizationId)(user);
        }
        catch (authError) {
    ...
    try {
        ...
        for (const license of licenses) {
            const issues = [];
            let status = 'VALID';
            const daysUntilExpiration = Math.ceil((license.valid_until.getTime() - now.getTime()) / (1000 * 60 * 60 * 24));
        ...
        if (isScheduled) {
            logging_js_1.logger.info('Daily license validation completed:', report);
            return;
        }
        return (0, response_js_1.success)(report, 200, origin);
    }
    catch (err) {
        logging_js_1.logger.error('Daily license validation error:', err);
        if (isScheduled)
            return;
        return (0, response_js_1.error)('Internal server error', 500, undefined, origin);
    }
}
async function sendLicenseAlert(result) {
    const prisma = (0, database_js_1.getPrismaClient)();
...
//# sourceMappingURL=daily-license-validation.js.map