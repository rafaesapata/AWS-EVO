...
exports.advancedAnalytics = exports.AdvancedAnalytics = void 0;
...
class AdvancedAnalytics {
    ...
    async executeQuery(query) {
        ...
        if (cached) {
            logging_1.logger.debug('Analytics query cache hit', { cacheKey });
            return {
                ...cached,
                metadata: {
                    ...cached.metadata,
        ...
        try {
            logging_1.logger.info('Executing analytics query', {
            ...
            logging_1.logger.error('Analytics query execution failed', error, {
     ...
     */
    async getSecurityAnalytics(organizationId, timeRange) {
        try {
            logging_1.logger.info('Generating security analytics', { organizationId, timeRange });
            // Get findings data
            ...
            const topThreats = Object.entries(threatCounts)
                ...
                type,
                count,
                severity: this.calculateThreatSeverity(findings.filter(f => `${f.service}-${f.category}` === type)),
            ...
            const complianceStatus = complianceFrameworks.reduce((acc, framework) => {
                const frameworkFindings = findings.filter(f => f.compliance.some(c => c.includes(framework)));
            ...
            logging_1.logger.error('Security analytics generation failed', error, { organizationId });
            throw error;
        }
    }
    /**
     ...
     */
    async getCostAnalytics(organizationId, timeRange) {
        try {
            logging_1.logger.info('Generating cost analytics', { organizationId, timeRange });
            ...
            logging_1.logger.error('Cost analytics generation failed', error, { organizationId });
            throw error;
        }
    }
    /**
     ...
     */
    async getPerformanceAnalytics(organizationId, timeRange) {
        try {
            logging_1.logger.info('Generating performance analytics', { organizationId, timeRange });
            ...
            logging_1.logger.error('Performance analytics generation failed', error, { organizationId });
            throw error;
        }
    }
    /**
     ...
     */
    async generateAIInsights(query, data) {
        const insights = [];
        try {
            // Trend analysis
            if (data.length > 10) {
            ...
            logging_1.logger.error('AI insights generation failed', error);
        }
        return insights;
    }
    /**
     ...
     */
    generateRecommendations(query, data) {
        const insights = [];
    ...
    async executeRawQuery(query) {
        ...
        try {
            ...
            const results = await prisma[config.table].findMany({
                where,
    ...
    calculateSecurityScore(findings) {
        ...
        const totalWeight = findings.reduce((sum, finding) => {
            return sum + (weights[finding.severity] || 1);
        }, 0);
        return Math.max(0, 100 - totalWeight);
    }
    async calculateSecurityTrend(organizationId, timeRange) {
        const prisma = (0, database_1.getPrismaClient)();
        ...
        const calcScore = (findings) => findings.reduce((sum, f) => sum + (severityWeights[f.severity] || 0), 0);
    ...
    calculateThreatSeverity(findings) {
    ...
    projectFutureCostsFromGrouped(dailyCosts) {
        if (dailyCosts.length === 0)
            return 0;
        const totalCost = dailyCosts.reduce((sum, cost) => sum + Number(cost._sum?.cost || 0), 0);
    ...
    calculateCostTrendFromGrouped(dailyCosts) {
        if (dailyCosts.length < 2)
            return 0;
    ...
    generateTrendData(timeRange, minValue, maxValue) {
        const points = 20;
...
exports.advancedAnalytics = new AdvancedAnalytics();
