...
exports.advancedSecurityScanner = exports.AdvancedSecurityScanner = void 0;
...
class AdvancedSecurityScanner {
     ...
     */
    async performSecurityScan(config) {
        logging_1.logger.info('Starting advanced security scan', {
        ...
        try {
            ...
            logging_1.logger.info('Advanced security scan completed', {
                scanId: scan.id,
            ...
            logging_1.logger.error('Advanced security scan failed', error, {
                scanId: scan.id,
                organizationId: config.organizationId
            });
            throw error;
     ...
     */
    async performScanType(scanType, config) {
        const findings = [];
        switch (scanType) {
     ...
     */
    async scanNetworkSecurity(config) {
        const findings = [];
        for (const region of config.regions) {
            try {
                ...
                for (const sg of securityGroups) {
                    ...
                    const permissiveRules = sg.IpPermissions?.filter(rule => rule.IpRanges?.some(range => range.CidrIp === '0.0.0.0/0' || range.CidrIp === '::/0')) || [];
                    if (permissiveRules.length > 0) {
                        const criticalPorts = permissiveRules.filter(rule => (rule.FromPort && [22, 3389, 1433, 3306, 5432, 27017, 6379].includes(rule.FromPort)));
                ...
                logging_1.logger.error('Network security scan failed for region', error, { region });
            }
        }
        return findings;
    }
     ...
     * Scan IAM security
     */
    async scanIAMSecurity(config) {
        const findings = [];
        try {
            const iamClient = new client_iam_1.IAMClient({ region: 'us-east-1' }); // IAM is global
            // Scan IAM users
            ...
            for (const user of users) {
                ...
                try {
                    const attachedPolicies = await iamClient.send(new client_iam_1.ListAttachedUserPoliciesCommand({ UserName: user.UserName }));
                    const adminPolicies = attachedPolicies.AttachedPolicies?.filter(policy => policy.PolicyName?.toLowerCase().includes('admin') ||
                    ...
                    logging_1.logger.warn('Failed to check IAM user policies', { userName: user.UserName });
                }
            }
            // Scan IAM roles
            const rolesResponse = await iamClient.send(new client_iam_1.ListRolesCommand({}));
            ...
            for (const role of roles) {
                ...
                if (role.AssumeRolePolicyDocument) {
                    try {
                        ...
                        logging_1.logger.warn('Failed to parse assume role policy', { roleName: role.RoleName });
                    }
                }
            }
        }
        catch (error) {
            logging_1.logger.error('IAM security scan failed', error);
        }
        return findings;
    }
    /**
     ...
     */
    async scanDataProtection(config) {
        const findings = [];
        try {
            const s3Client = new client_s3_1.S3Client({ region: 'us-east-1' }); // S3 is global
            // Scan S3 buckets
            ...
            for (const bucket of buckets) {
                ...
                try {
                    ...
                    const publicAccessBlock = await s3Client.send(new client_s3_1.GetPublicAccessBlockCommand({ Bucket: bucket.Name }));
                    ...
                    // Check encryption
                    try {
                        await s3Client.send(new client_s3_1.GetBucketEncryptionCommand({ Bucket: bucket.Name }));
                    }
                    ...
                    logging_1.logger.warn('Failed to check S3 bucket configuration', { bucketName: bucket.Name });
                }
            }
        }
        catch (error) {
            logging_1.logger.error('Data protection scan failed', error);
        }
        return findings;
    }
    /**
     ...
     */
    async scanLoggingMonitoring(config) {
        const findings = [];
        try {
            const cloudTrailClient = new client_cloudtrail_1.CloudTrailClient({ region: 'us-east-1' });
            // Check CloudTrail configuration
            const trailsResponse = await cloudTrailClient.send(new client_cloudtrail_1.DescribeTrailsCommand({}));
            ...
            logging_1.logger.error('Logging and monitoring scan failed', error);
        }
        return findings;
    }
    /**
     ...
     */
    async scanThreatDetection(config) {
        const findings = [];
        for (const region of config.regions) {
            try {
                ...
                logging_1.logger.warn('GuardDuty check failed for region', { region, error: error.message });
            }
        }
        return findings;
    }
     ...
     */
    async scanVulnerabilities(config) {
     ...
     */
    async scanConfigurationDrift(config) {
     ...
     */
    async enhanceWithAIAnalysis(findings) {
    ...
    generateAIAnalysis(finding) {
        ...
        const template = templates[finding.severity] || templates.medium;
        return template.join('');
    }
    /**
     * Calculate security posture
     */
    async calculateSecurityPosture(organizationId, findings) {
        ...
        const weightedScore = totalFindings === 0 ? 100 : Math.max(0, 100 - ((findingsSummary.critical * 25) +
...
exports.advancedSecurityScanner = new AdvancedSecurityScanner();
//# sourceMappingURL=advanced-security-scanner.js.map