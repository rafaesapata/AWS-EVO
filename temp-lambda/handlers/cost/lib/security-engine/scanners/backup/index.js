...
exports.BackupScanner = void 0;
exports.scanBackup = scanBackup;
const base_scanner_js_1 = require("../../core/base-scanner.js");
const client_backup_1 = require("@aws-sdk/client-backup");
class BackupScanner extends base_scanner_js_1.BaseScanner {
    get serviceName() { return 'Backup'; }
    ...
    async scan() {
        ...
        try {
            // Check backup vaults
            const vaults = await client.send(new client_backup_1.ListBackupVaultsCommand({}));
            ...
            for (const vault of vaults.BackupVaultList || []) {
                ...
                await this.safeExecute(`vault-details-${vault.BackupVaultName}`, async () => {
                    const vaultDetails = await client.send(new client_backup_1.DescribeBackupVaultCommand({
                ...
                try {
                    await client.send(new client_backup_1.GetBackupVaultAccessPolicyCommand({
                        BackupVaultName: vault.BackupVaultName
                    }));
                }
                catch (e) {
            ...
            // Check backup plans
            const plans = await client.send(new client_backup_1.ListBackupPlansCommand({}));
            ...
            for (const plan of plans.BackupPlansList || []) {
                if (!plan.BackupPlanId || !plan.BackupPlanArn)
                    continue;
                await this.safeExecute(`plan-details-${plan.BackupPlanId}`, async () => {
                    const planDetails = await client.send(new client_backup_1.GetBackupPlanCommand({
                        BackupPlanId: plan.BackupPlanId
                    }));
                    // Check 4: No cross-region copy
                    const hasCrossRegion = planDetails.BackupPlan?.Rules?.some(rule => rule.CopyActions?.some(copy => copy.DestinationBackupVaultArn));
                    if (!hasCrossRegion) {
                        findings.push(this.createFinding({
                            severity: 'low',
                    ...
                    const shortRetention = planDetails.BackupPlan?.Rules?.some(rule => rule.Lifecycle?.DeleteAfterDays && rule.Lifecycle.DeleteAfterDays < 30);
                    if (shortRetention) {
                        findings.push(this.createFinding({
                            severity: 'low',
            ...
            await this.safeExecute('protected-resources', async () => {
                const protectedResources = await client.send(new client_backup_1.ListProtectedResourcesCommand({}));
            ...
            this.warn('Backup scan failed', { error: error.message });
        }
        this.log('AWS Backup scan completed', { findingsCount: findings.length });
        return findings;
    }
}
exports.BackupScanner = BackupScanner;
async function scanBackup(region, accountId, credentials, cache) {
