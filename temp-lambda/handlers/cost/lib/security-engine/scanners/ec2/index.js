...
exports.EC2Scanner = void 0;
exports.scanEC2 = scanEC2;
const base_scanner_js_1 = require("../../core/base-scanner.js");
const config_js_1 = require("../../config.js");
const client_ec2_1 = require("@aws-sdk/client-ec2");
class EC2Scanner extends base_scanner_js_1.BaseScanner {
    get serviceName() {
        return 'EC2';
    }
    get category() {
    ...
    async checkSecurityGroups(client) {
        const findings = [];
        try {
            const response = await client.send(new client_ec2_1.DescribeSecurityGroupsCommand({}));
                ...
                for (const rule of sg.IpPermissions || []) {
                    for (const ipRange of rule.IpRanges || []) {
                        if (ipRange.CidrIp === '0.0.0.0/0') {
                            ...
                            for (const [port, service] of Object.entries(config_js_1.CRITICAL_PORTS)) {
                            ...
                            for (const [port, service] of Object.entries(config_js_1.HIGH_RISK_PORTS)) {
                                const portNum = parseInt(port);
                                if (fromPort <= portNum && toPort >= portNum) {
                                    findings.push(this.createFinding({
                                        severity: 'high',
            ...
            this.warn('Failed to check security groups', { error: error.message });
        }
        return findings;
    }
    async checkInstances(client) {
        const findings = [];
        try {
            ...
            this.warn('Failed to check instances', { error: error.message });
        }
        return findings;
    }
    async checkVolumes(client) {
        const findings = [];
        try {
            const response = await client.send(new client_ec2_1.DescribeVolumesCommand({}));
            ...
            this.warn('Failed to check volumes', { error: error.message });
        }
        return findings;
    }
    async checkSnapshots(client) {
        const findings = [];
        try {
            const response = await client.send(new client_ec2_1.DescribeSnapshotsCommand({ OwnerIds: ['self'] }));
    ...
    async checkVPCFlowLogs(client) {
        const findings = [];
        try {
            const vpcsResponse = await client.send(new client_ec2_1.DescribeVpcsCommand({}));
            const flowLogsResponse = await client.send(new client_ec2_1.DescribeFlowLogsCommand({}));
            const vpcWithFlowLogs = new Set((flowLogsResponse.FlowLogs || [])
            ...
            this.warn('Failed to check VPC flow logs', { error: error.message });
        }
        return findings;
    }
}
exports.EC2Scanner = EC2Scanner;
async function scanEC2(region, accountId, credentials, cache) {
