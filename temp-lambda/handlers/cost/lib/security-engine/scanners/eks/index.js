"use strict";
/**
 * Security Engine V2 - EKS Scanner
 */
Object.defineProperty(exports, "__esModule", { value: true });
exports.EKSScanner = void 0;
exports.scanEKS = scanEKS;
const base_scanner_js_1 = require("../../core/base-scanner.js");
const client_eks_1 = require("@aws-sdk/client-eks");
class EKSScanner extends base_scanner_js_1.BaseScanner {
    get serviceName() { return 'EKS'; }
    ...
    async scan() {
        ...
        try {
            const response = await client.send(new client_eks_1.ListClustersCommand({}));
            ...
            this.warn('Failed to list clusters', { error: error.message });
        }
        this.log('EKS scan completed', { findingsCount: findings.length });
        return findings;
    }
    async checkCluster(client, clusterName) {
        const findings = [];
        const clusterArn = this.arnBuilder.eksCluster(this.region, clusterName);
        try {
            const details = await client.send(new client_eks_1.DescribeClusterCommand({ name: clusterName }));
            ...
            const enabledLogs = logging.filter((l) => l.enabled).flatMap((l) => l.types || []);
            ...
            this.warn(`Failed to describe cluster ${clusterName}`, { error: error.message });
        }
        return findings;
    }
}
exports.EKSScanner = EKSScanner;
async function scanEKS(region, accountId, credentials, cache) {
