...
exports.EventBridgeScanner = void 0;
exports.scanEventBridge = scanEventBridge;
const base_scanner_js_1 = require("../../core/base-scanner.js");
const client_eventbridge_1 = require("@aws-sdk/client-eventbridge");
class EventBridgeScanner extends base_scanner_js_1.BaseScanner {
    get serviceName() { return 'EventBridge'; }
    ...
    async scan() {
        ...
        try {
            // Check event buses
            const buses = await client.send(new client_eventbridge_1.ListEventBusesCommand({}));
            for (const bus of buses.EventBuses || []) {
                ...
                if (bus.Policy) {
                    try {
                        ...
                        const hasPublicAccess = policy.Statement?.some((stmt) => stmt.Principal === '*' && !stmt.Condition);
                        if (hasPublicAccess) {
            ...
            for (const rule of rules.Rules || []) {
                ...
                await this.safeExecute(`targets-${rule.Name}`, async () => {
                    const targets = await client.send(new client_eventbridge_1.ListTargetsByRuleCommand({ Rule: rule.Name }));
            ...
            this.warn('EventBridge scan failed', { error: error.message });
        }
        this.log('EventBridge scan completed', { findingsCount: findings.length });
        return findings;
    }
}
exports.EventBridgeScanner = EventBridgeScanner;
async function scanEventBridge(region, accountId, credentials, cache) {
