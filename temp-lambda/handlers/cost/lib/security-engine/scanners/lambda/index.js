...
exports.LambdaScanner = void 0;
exports.scanLambda = scanLambda;
const base_scanner_js_1 = require("../../core/base-scanner.js");
const config_js_1 = require("../../config.js");
const client_lambda_1 = require("@aws-sdk/client-lambda");
class LambdaScanner extends base_scanner_js_1.BaseScanner {
    get serviceName() {
        return 'Lambda';
    }
    get category() {
    ...
    async scan() {
        ...
        try {
            const functionsResponse = await lambdaClient.send(new client_lambda_1.ListFunctionsCommand({}));
            ...
            this.warn('Failed to list Lambda functions', { error: error.message });
        }
        this.log('Lambda scan completed', { findingsCount: findings.length });
        return findings;
    }
    async scanFunction(client, fn) {
        const findings = [];
        ...
        await this.safeExecute('functionUrl', async () => {
            try {
                const urlConfig = await client.send(new client_lambda_1.GetFunctionUrlConfigCommand({ FunctionName: functionName }));
        ...
        for (const key of Object.keys(envVars)) {
            if (config_js_1.SENSITIVE_ENV_PATTERNS.some(pattern => pattern.test(key))) {
        ...
        if (fn.Runtime && config_js_1.DEPRECATED_RUNTIMES.includes(fn.Runtime)) {
            findings.push(this.createFinding({
                severity: 'high',
        ...
        await this.safeExecute('resourcePolicy', async () => {
            try {
                const policyResponse = await client.send(new client_lambda_1.GetPolicyCommand({ FunctionName: functionName }));
...
exports.LambdaScanner = LambdaScanner;
async function scanLambda(region, accountId, credentials, cache) {
