...
exports.identifyPeakHours = identifyPeakHours;
exports.getOptimalSchedulingWindows = getOptimalSchedulingWindows;
/**
 ...
 */
function detectSeasonality(datapoints) {
    ...
    const hourlyAverages = hourlyTotals.map((total, i) => hourlyCounts[i] > 0 ? total / hourlyCounts[i] : 0);
    const weekdayAverages = weekdayTotals.map((total, i) => weekdayCounts[i] > 0 ? total / weekdayCounts[i] : 0);
    ...
    const hourlyStdDev = Math.sqrt(hourlyAverages.reduce((sum, v) => sum + (v - hourlyMean) ** 2, 0) / 24);
    ...
    const weekdayStdDev = Math.sqrt(weekdayAverages.reduce((sum, v) => sum + (v - weekdayMean) ** 2, 0) / 7);
 ...
 */
function identifyPeakHours(hourlyValues, topN = 4) {
 ...
 */
function getOptimalSchedulingWindows(seasonality) {
...
//# sourceMappingURL=seasonality-detector.js.map