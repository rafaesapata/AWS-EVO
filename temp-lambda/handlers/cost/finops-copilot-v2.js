...
async function handler(event, context) {
    ...
    try {
        ...
        const parseResult = schemas_js_1.finopsCopilotSchema.safeParse(event.body ? JSON.parse(event.body) : {});
        ...
        if (!awsAccount) {
            return (0, response_js_1.error)('AWS Account not found', 404, undefined, origin);
        }
        ...
        logging_js_1.logger.error('FinOps Copilot V2 error:', error);
        return {
            statusCode: 500,
...
;
async function collectCostData(ceClient, timeRange) {
    ...
    // Custo por serviço
    const serviceResponse = await ceClient.send(new client_cost_explorer_1.GetCostAndUsageCommand({
    ...
    // Custo por região
    const regionResponse = await ceClient.send(new client_cost_explorer_1.GetCostAndUsageCommand({
        TimePeriod: { Start: start, End: end },
        Granularity: 'MONTHLY',
        Metrics: ['UnblendedCost'],
    ...
    // Tendência diária
    const trendResponse = await ceClient.send(new client_cost_explorer_1.GetCostAndUsageCommand({
        TimePeriod: { Start: start, End: end },
        Granularity: 'DAILY',
        Metrics: ['UnblendedCost']
    }));
    ...
    try {
        ...
        const forecastResponse = await ceClient.send(new client_cost_explorer_1.GetCostForecastCommand({
        ...
        logging_js_1.logger.info('Forecast not available');
    }
    return { totalCost, byService, byRegion, trend, forecast };
}
function buildAIContext(costData, optimizations, alerts, accountName) {
    return `
...
async function generateAIResponse(question, context, queryContext) {
...
Format your response as JSON with: answer, insights (array), recommendations (array), sources (array).`;
    try {
        ...
        logging_js_1.logger.error('Bedrock error:', error);
        return {
...
//# sourceMappingURL=finops-copilot-v2.js.map