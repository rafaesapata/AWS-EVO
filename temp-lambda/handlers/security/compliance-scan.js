...
async function handler(event, context) {
    ...
    logging_js_1.logger.info('Compliance scan started');
    ...
    try {
        ...
        const parseResult = schemas_js_1.complianceScanSchema.safeParse(event.body ? JSON.parse(event.body) : {});
        ...
        logging_js_1.logger.info('Starting compliance scan', { frameworkId });
        ...
        logging_js_1.logger.info('Analyzing compliance controls', { frameworkId, controlCount: controls.length });
        // Analyze compliance based on findings
        const complianceResults = analyzeCompliance(frameworkId, controls, findings, posture);
        ...
        logging_js_1.logger.info('Compliance scan completed', { passed, failed, total: complianceResults.length });
        ...
        logging_js_1.logger.error('Compliance scan error', err);
 ...
 */
function analyzeCompliance(frameworkId, controls, findings, posture) {
    const results = [];
    for (const control of controls) {
        ...
        if (control.control_id.includes('CIS-1')) {
            // IAM controls
            const iamFindings = findings.filter(f => f.service === 'IAM' || f.scan_type?.includes('iam'));
            ...
            // Logging controls
            const loggingFindings = findings.filter(f => f.scan_type?.includes('cloudtrail') || f.scan_type?.includes('logging'));
            ...
            // Network controls
            const networkFindings = findings.filter(f => f.category === 'Network Exposure' ||
            ...
            // Storage controls
            const storageFindings = findings.filter(f => f.service === 'S3' || f.service === 'RDS' || f.service === 'EBS');
            ...
            const encryptionFindings = findings.filter(f => f.scan_type?.includes('encryption') || f.scan_type?.includes('public'));
            ...
            // PCI-DSS controls
            const pciFindings = findings.filter(f => f.compliance?.includes('PCI-DSS'));
...
//# sourceMappingURL=compliance-scan.js.map