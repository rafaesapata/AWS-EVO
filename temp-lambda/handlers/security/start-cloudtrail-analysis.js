 ...
 */
async function checkPeriodOverlap(prisma, organizationId, accountId, periodStart, periodEnd) {
...
async function handler(event, _context) {
    ...
    if (!prisma) {
        logging_js_1.logger.error('Prisma client is undefined');
        return (0, response_js_1.error)('Database client initialization failed', 500, undefined, origin);
    }
    ...
    while (connectionAttempts < maxAttempts) {
        try {
            ...
            logging_js_1.logger.info('Database connection established successfully');
            break;
        }
        catch (dbError) {
            connectionAttempts++;
    ...
    try {
        ...
        if (hoursBack > 2880) {
            return (0, response_js_1.badRequest)('Maximum analysis period is 120 days (2880 hours)', undefined, origin);
        }
        if (!accountId) {
        ...
        if (!account) {
            return (0, response_js_1.badRequest)('AWS account not found', undefined, origin);
        }
        ...
        const overlapInfo = await checkPeriodOverlap(prisma, organizationId, accountId, periodStart, periodEnd);
        // If period is fully covered and not forcing reprocess, return info
        if (overlapInfo.coveragePercent >= 95 && !forceReprocess) {
            logging_js_1.logger.info('Period already processed', {
                organizationId,
                accountId,
                periodStart,
                periodEnd,
        ...
        logging_js_1.logger.info('Starting CloudTrail analysis async', {
            analysisId,
            organizationId,
            accountId,
            hoursBack,
        ...
        logging_js_1.logger.info('CloudTrail analysis Lambda invoked async', { analysisId });
        return (0, response_js_1.success)({
            success: true,
            analysisId,
            status: 'running',
        ...
        logging_js_1.logger.error('Start CloudTrail analysis error', err);
        return (0, response_js_1.error)(err instanceof Error ? err.message : 'Internal server error', 500, undefined, origin);
    }
}
//# sourceMappingURL=start-cloudtrail-analysis.js.map