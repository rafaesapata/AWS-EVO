...
async function handler(event, context) {
    ...
    try {
        ...
        logging_js_1.logger.info('Cleanup initiated by user', { userId: user.sub, organizationId });
    }
    catch (authError) {
        // Execução administrativa (via CloudWatch Events, etc.)
        isAdminExecution = true;
        logging_js_1.logger.info('Cleanup initiated administratively');
    }
    ...
    try {
        ...
        logging_js_1.logger.info('Starting cleanup of stuck scans', {
        ...
        logging_js_1.logger.info('Found stuck scans', { count: stuckScans.length });
        ...
        for (const scan of stuckScans) {
            try {
                ...
                logging_js_1.logger.info('Processing stuck scan', {
                    scanId: scan.id,
                ...
                stats.scansUpdated++;
                logging_js_1.logger.info('Updated stuck scan to failed status', {
                    scanId: scan.id,
                    durationMinutes
                });
            }
                ...
                logging_js_1.logger.error('Error updating stuck scan', {
                    scanId: scan.id,
                    error: errorMsg
                });
            }
        ...
        logging_js_1.logger.error('Cleanup job failed', {
            error: errorMessage,
