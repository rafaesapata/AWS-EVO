...
const redis_cache_js_1 = require("../../lib/redis-cache.js");
...
async function handler(event, _context) {
    logging_js_1.logger.info('🚀 Fetch Edge Services started');
    ...
    try {
        ...
        if (!account) {
            return (0, response_js_1.error)('AWS account not found or inactive');
        }
        // ============================================
        ...
        if (!forceRefresh) {
            try {
                const cacheInfo = await redis_cache_js_1.edgeCache.getCacheInfo(accountId);
                if (cacheInfo.hasCache) {
                    logging_js_1.logger.info(`📦 Cache HIT for account ${accountId}, age: ${cacheInfo.cacheAge}s`);
                    // Retornar dados do cache se ainda válido
                    const cached = await redis_cache_js_1.edgeCache.getDiscoveryResult(accountId);
                    if (cached) {
                ...
                logging_js_1.logger.warn('Redis cache check failed, proceeding without cache', { error: cacheError instanceof Error ? cacheError.message : String(cacheError) });
            }
        }
        else {
            logging_js_1.logger.info(`🔄 Force refresh requested, bypassing cache for account ${accountId}`);
        }
        logging_js_1.logger.info(`📊 Discovering edge services for account ${account.account_name}`);
        const allServices = [];
        const permissionErrors = [];
        // CloudFront é global - usar us-east-1
        const globalCreds = await (0, aws_helpers_js_1.resolveAwsCredentials)(account, 'us-east-1');
        const globalCredentials = (0, aws_helpers_js_1.toAwsCredentials)(globalCreds);
        ...
        logging_js_1.logger.info(`📦 Found ${allServices.length} edge services`);
        ...
        for (const service of allServices) {
            ...
            if (savedService) {
                ...
                const resolvedCreds = await (0, aws_helpers_js_1.resolveAwsCredentials)(account, metricsRegion);
                const credentials = (0, aws_helpers_js_1.toAwsCredentials)(resolvedCreds);
        ...
        logging_js_1.logger.info(`✅ Saved ${allServices.length} services, collected ${metricsCollected} metrics`);
        const breakdown = {
        ...
        // ============================================
        try {
            await redis_cache_js_1.edgeCache.cacheDiscoveryResult(accountId, {
            ...
            logging_js_1.logger.info(`📦 Cached discovery result for account ${accountId}, TTL: ${CACHE_TTL}s`);
        }
        catch (cacheError) {
            // Se Redis falhar ao salvar, apenas logar
            logging_js_1.logger.warn('Failed to cache discovery result', { error: cacheError instanceof Error ? cacheError.message : String(cacheError) });
        }
        return (0, response_js_1.success)({
            success: true,
        ...
        logging_js_1.logger.error('❌ Fetch Edge Services error:', err);
        return (0, response_js_1.error)(err instanceof Error ? err.message : 'Internal server error');
    }
}
// Descobrir CloudFront distributions
async function discoverCloudFrontDistributions(credentials, permissionErrors) {
    try {
        const client = new client_cloudfront_1.CloudFrontClient({ region: 'us-east-1', credentials });
        ...
        logging_js_1.logger.warn(`CloudFront discovery failed: ${err.message}`);
...
async function discoverWAFWebACLs(credentials, scope, region, permissionErrors) {
    try {
        const client = new client_wafv2_1.WAFV2Client({
            region: scope === 'CLOUDFRONT' ? 'us-east-1' : region,
            credentials
        });
        const response = await client.send(new client_wafv2_1.ListWebACLsCommand({ Scope: scope }));
        const services = [];
        ...
        logging_js_1.logger.warn(`WAF discovery failed in ${region}: ${err.message}`);
...
async function discoverLoadBalancers(credentials, region, permissionErrors) {
    try {
        ...
        logging_js_1.logger.warn(`ELB discovery failed in ${region}: ${err.message}`);
        if (err.name === 'AccessDeniedException') {
            permissionErrors.push({
                serviceType: 'load_balancer',
                region,
...
async function collectEdgeMetrics(credentials, region, service) {
    try {
        ...
        logging_js_1.logger.warn(`Failed to collect metrics for ${service.serviceId}: ${err}`);
        return null;
    }
}
// Coletar métricas do CloudFront
async function collectCloudFrontMetrics(client, distributionId, startTime, endTime) {
    const dimensions = [
...
async function collectWAFMetrics(client, service, startTime, endTime) {
...
async function collectLoadBalancerMetrics(client, service, startTime, endTime) {
...
async function getMetricSum(client, namespace, metricName, dimensions, startTime, endTime) {
    try {
...
async function getMetricAvg(client, namespace, metricName, dimensions, startTime, endTime) {
    try {
        const response = await client.send(new client_cloudwatch_1.GetMetricStatisticsCommand({
            Namespace: namespace,
            MetricName: metricName,
...
async function upsertEdgeService(prisma, organizationId, accountId, service) {
    try {
        ...
        logging_js_1.logger.warn(`Failed to upsert edge service ${service.serviceId}: ${err}`);
        return null;
    }
}
// Salvar métricas de borda no banco
async function saveEdgeMetrics(prisma, organizationId, accountId, serviceId, metrics) {
    try {
        ...
        if (!err?.code?.includes('P2002')) {
            logging_js_1.logger.warn(`Failed to save edge metrics: ${err}`);
        }
    }
}
// Atualizar estatísticas do serviço
async function updateServiceStats(prisma, serviceId, metrics) {
    try {
        ...
        logging_js_1.logger.warn(`Failed to update service stats: ${err}`);
    }
}
//# sourceMappingURL=fetch-edge-services.js.map