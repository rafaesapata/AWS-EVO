...
async function handler(event) {
    logging_js_1.logger.info('Starting database migration');
    ...
    if (event) {
        ...
        try {
            ...
            logging_js_1.logger.warn('Unauthorized migration attempt');
            return (0, response_js_1.error)('Unauthorized', 401);
        }
        if (!(0, auth_js_1.isSuperAdmin)(user)) {
            logging_js_1.logger.warn('Forbidden migration attempt', { userId: user.sub });
            return (0, response_js_1.error)('Forbidden - Super admin required', 403);
        }
        logging_js_1.logger.info('Migration authorized', { userId: user.sub });
    }
    try {
        const prisma = (0, database_js_1.getPrismaClient)();
        // Test connection
        await prisma.$queryRaw `SELECT 1 as test`;
        logging_js_1.logger.info('Database connection successful');
        ...
        for (let i = 0; i < MIGRATION_COMMANDS.length; i++) {
            ...
            try {
                ...
                logging_js_1.logger.info(`Migration command ${i + 1} executed successfully`);
            }
            catch (err) {
                // Log but continue on non-critical errors
                results.push(`Command ${i + 1}: ${err.message}`);
                logging_js_1.logger.warn(`Migration command ${i + 1} warning: ${err.message}`);
            }
        }
        // Check tables
    ...
    `;
        logging_js_1.logger.info('Tables after migration', { tables: tables.map(t => t.tablename) });
        return {
            statusCode: 200,
            headers: {
        ...
        logging_js_1.logger.error('Migration error', err);
        return {
            statusCode: 500,
            headers: {
                'Content-Type': 'application/json',
