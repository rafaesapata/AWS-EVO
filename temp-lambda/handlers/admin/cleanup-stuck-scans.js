...
async function handler(event, _context) {
    ...
    try {
        ...
        userOrganizationId = (0, auth_js_1.getOrganizationId)(user);
    }
    catch (authError) {
        return (0, response_js_1.unauthorized)('Authentication required for manual cleanup');
    }
    ...
    if (!isAdmin) {
        return (0, response_js_1.unauthorized)('Admin permission required for cleanup operations');
    }
    const prisma = (0, database_js_1.getPrismaClient)();
    try {
        // Parse do body da requisição
        const body = (0, request_parser_js_1.parseEventBody)(event, {}, 'cleanup-stuck-scans');
        ...
        if (!isSuperAdmin && targetOrganizationId !== userOrganizationId) {
            return (0, response_js_1.unauthorized)('Cannot cleanup scans from other organizations');
        }
        const stuckThreshold = new Date(Date.now() - thresholdMinutes * 60 * 1000);
        logging_js_1.logger.info('Manual cleanup initiated', {
            userId: user.sub,
            userOrganizationId,
        ...
        for (const scan of stuckScans) {
            try {
                ...
                stats.scansUpdated++;
                logging_js_1.logger.info('Manually cleaned up stuck scan', {
                    scanId: scan.id,
                    userId: user.sub,
                    durationMinutes
                });
                ...
                logging_js_1.logger.error('Error in manual cleanup', {
                    scanId: scan.id,
                    userId: user.sub,
                    error: errorMsg
                });
        ...
        logging_js_1.logger.info('Manual cleanup completed', {
            userId: user.sub,
            targetOrganizationId,
        ...
        logging_js_1.logger.error('Manual cleanup failed', {
            error: errorMessage,
            userId: user.sub,
            userOrganizationId
        });
        return (0, response_js_1.error)(`Manual cleanup failed: ${errorMessage}`, 500);
    }
}
//# sourceMappingURL=cleanup-stuck-scans.js.map