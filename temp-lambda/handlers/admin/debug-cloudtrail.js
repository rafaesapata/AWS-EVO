...
async function handler(event, context) {
    ...
    try {
        logging_js_1.logger.info('Debug CloudTrail - Starting analysis', { organizationId });
        ...
        logging_js_1.logger.info('Debug CloudTrail - Analysis complete', debugInfo);
        return (0, response_js_1.success)(debugInfo);
    }
    catch (err) {
        const errorMessage = err instanceof Error ? err.message : 'Unknown error during debug';
        logging_js_1.logger.error('Debug CloudTrail failed', { error: errorMessage, organizationId });
        return (0, response_js_1.error)(`Debug failed: ${errorMessage}`, 500);
    }
}
//# sourceMappingURL=debug-cloudtrail.js.map