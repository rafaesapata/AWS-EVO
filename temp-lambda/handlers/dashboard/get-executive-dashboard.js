...
const requestSchema = zod_1.z.object({
    ...
    includeTrends: zod_1.z.boolean().default(true),
    includeInsights: zod_1.z.boolean().default(true),
    trendPeriod: zod_1.z.enum(['7d', '30d', '90d']).default('30d')
});
// ============================================================================
// HANDLER
// ============================================================================
async function handler(event, context) {
    ...
    try {
        ...
        logging_js_1.logger.info('Executive Dashboard request', {
            organizationId,
            userId,
            params,
        ...
        const [financialData, securityData, operationsData, insightsData, trendsData] = await Promise.all([
        ...
        logging_js_1.logger.info('Executive Dashboard generated', {
            organizationId,
            overallScore: summary.overallScore,
            executionTime,
            requestId: context.awsRequestId
        });
        return (0, response_js_1.success)(response, 200, origin);
    }
    catch (err) {
        logging_js_1.logger.error('Executive Dashboard error', err, {
            requestId: context.awsRequestId
        });
        if (err instanceof zod_1.z.ZodError) {
            return (0, response_js_1.badRequest)('Invalid request parameters', undefined, origin);
        }
        // Never expose internal error details to the client
        return (0, response_js_1.error)('Unable to load dashboard data. Please try again later.', 500, undefined, origin);
    }
}
// ============================================================================
// DATA FUNCTIONS
// ============================================================================
async function getFinancialData(prisma, organizationId, _accountId, // Not used - daily_costs doesn't filter by account
startOfMonth, startOfYear) {
    ...
    const sortedServices = topServices.map((item, index) => ({
...
async function getSecurityData(prisma, organizationId) {
...
async function getOperationsData(prisma, organizationId) {
...
async function getInsightsData(prisma, organizationId) {
    // AI Insights table doesn't exist yet - return empty array
    // Future: implement AI insights generation
    return [];
}
async function getTrendsData(prisma, organizationId, period) {
...
// HELPER FUNCTIONS
// ============================================================================
function calculateExecutiveSummary(financial, security, operations) {
    ...
    const overallScore = Math.round((security.score * 0.4) +
...
//# sourceMappingURL=get-executive-dashboard.js.map