        ...
        try {
            ...
            prisma.$use(async (params, next) => {
                ...
                if (isProduction && duration > 1000) {
                    logging_1.logger.warn('Slow database query detected', {
                        model: params.model,
                ...
                if (isProduction && ['create', 'update', 'delete', 'createMany', 'updateMany', 'deleteMany'].includes(params.action || '')) {
                    logging_1.logger.info('Database write operation', {
                        model: params.model,
                        action: params.action,
                        duration
                    });
            ...
            logging_1.logger.info('Prisma client initialized', { isLambda, isProduction, queryLogging: enableQueryLogging });
        }
        catch (error) {
            logging_1.logger.error('Failed to create Prisma client', error);
