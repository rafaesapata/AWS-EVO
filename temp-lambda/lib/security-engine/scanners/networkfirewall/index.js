...
exports.NetworkFirewallScanner = void 0;
exports.scanNetworkFirewall = scanNetworkFirewall;
...
class NetworkFirewallScanner extends base_scanner_js_1.BaseScanner {
    get serviceName() { return 'NetworkFirewall'; }
    ...
    async scan() {
        ...
        try {
            const firewalls = await client.send(new client_network_firewall_1.ListFirewallsCommand({}));
            ...
            for (const fw of firewalls.Firewalls || []) {
                if (!fw.FirewallArn || !fw.FirewallName)
                    continue;
                const fwName = fw.FirewallName;
                const fwArn = fw.FirewallArn;
                await this.safeExecute(`firewall-${fwName}`, async () => {
                    const details = await client.send(new client_network_firewall_1.DescribeFirewallCommand({
                        FirewallArn: fwArn
                    }));
                    ...
                    const logging = await client.send(new client_network_firewall_1.DescribeLoggingConfigurationCommand({
                        FirewallArn: fwArn
                    }));
                    if (!logging.LoggingConfiguration?.LogDestinationConfigs?.length) {
                        ...
                        const logTypes = logging.LoggingConfiguration.LogDestinationConfigs.map((c) => c.LogType);
                    ...
                    if (firewall.FirewallPolicyArn) {
                        const policy = await client.send(new client_network_firewall_1.DescribeFirewallPolicyCommand({
            ...
            this.warn('Network Firewall scan failed', { error: error.message });
        }
        this.log('Network Firewall scan completed', { findingsCount: findings.length });
        return findings;
    }
}
exports.NetworkFirewallScanner = NetworkFirewallScanner;
async function scanNetworkFirewall(region, accountId, credentials, cache) {
