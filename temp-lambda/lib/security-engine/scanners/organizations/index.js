...
exports.OrganizationsScanner = void 0;
exports.scanOrganizations = scanOrganizations;
const base_scanner_js_1 = require("../../core/base-scanner.js");
const client_organizations_1 = require("@aws-sdk/client-organizations");
class OrganizationsScanner extends base_scanner_js_1.BaseScanner {
    get serviceName() { return 'Organizations'; }
    ...
    async scan() {
        ...
        try {
            ...
            const org = await client.send(new client_organizations_1.DescribeOrganizationCommand({}));
            if (!org.Organization) {
            ...
            await this.safeExecute('scps', async () => {
                const scps = await client.send(new client_organizations_1.ListPoliciesCommand({
            ...
            await this.safeExecute('tag-policies', async () => {
                const tagPolicies = await client.send(new client_organizations_1.ListPoliciesCommand({
                    Filter: 'TAG_POLICY'
                }));
            ...
            await this.safeExecute('backup-policies', async () => {
                const backupPolicies = await client.send(new client_organizations_1.ListPoliciesCommand({
            ...
            await this.safeExecute('ai-optout-policies', async () => {
                const aiPolicies = await client.send(new client_organizations_1.ListPoliciesCommand({
...
exports.OrganizationsScanner = OrganizationsScanner;
async function scanOrganizations(region, accountId, credentials, cache) {
