"use strict";
/**
 * Security Engine V3 - Kinesis Scanner
 */
Object.defineProperty(exports, "__esModule", { value: true });
exports.KinesisScanner = void 0;
exports.scanKinesis = scanKinesis;
...
class KinesisScanner extends base_scanner_js_1.BaseScanner {
    get serviceName() { return 'Kinesis'; }
    ...
    async scan() {
        ...
        try {
            ...
            const streams = await kinesisClient.send(new client_kinesis_1.ListStreamsCommand({}));
            for (const streamName of streams.StreamNames || []) {
                await this.safeExecute(`stream-${streamName}`, async () => {
                    const details = await kinesisClient.send(new client_kinesis_1.DescribeStreamSummaryCommand({
            ...
            const firehoseStreams = await firehoseClient.send(new client_firehose_1.ListDeliveryStreamsCommand({}));
            for (const deliveryStreamName of firehoseStreams.DeliveryStreamNames || []) {
                await this.safeExecute(`firehose-${deliveryStreamName}`, async () => {
                    const details = await firehoseClient.send(new client_firehose_1.DescribeDeliveryStreamCommand({
            ...
            this.warn('Kinesis scan failed', { error: error.message });
        }
        this.log('Kinesis scan completed', { findingsCount: findings.length });
        return findings;
    }
}
exports.KinesisScanner = KinesisScanner;
async function scanKinesis(region, accountId, credentials, cache) {
