...
async function applyMigration() {
  try {
    console.log('🚀 Aplicando migração RI/SP via Lambda...');
    
    const result = await lambda.invoke({
      FunctionName: 'EvoUdsDevelopmentApiStack-HealthCheckFunctionC4679-Tr0alHlfS9on',
      Payload: JSON.stringify(migrationPayload)
    }).promise();
    
    const response = JSON.parse(result.Payload);
    console.log('✅ Migração aplicada:', response);
