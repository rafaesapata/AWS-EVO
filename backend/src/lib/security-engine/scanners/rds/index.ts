...
export class RDSScanner extends BaseScanner {
  get serviceName(): string { return 'RDS'; }
  ...
  private async checkDBInstances(client: RDSClient): Promise<Finding[]> {
    const findings: Finding[] = [];
    try {
      ...
      this.warn('Failed to check DB instances', { error: (error as Error).message });
    }
    return findings;
  }

  private async checkDBClusters(client: RDSClient): Promise<Finding[]> {
    const findings: Finding[] = [];
    try {
      ...
      this.warn('Failed to check DB clusters', { error: (error as Error).message });
    }
    return findings;
  }

  private async checkSnapshots(client: RDSClient): Promise<Finding[]> {
    const findings: Finding[] = [];
    try {
      const response = await client.send(new DescribeDBSnapshotsCommand({ SnapshotType: 'manual' }));
