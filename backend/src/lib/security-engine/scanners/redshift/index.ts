...
export class RedshiftScanner extends BaseScanner {
  get serviceName(): string { return 'Redshift'; }
    ...
    try {
      ...
      for (const cluster of clusters.Clusters || []) {
        ...
        await this.safeExecute(`logging-${cluster.ClusterIdentifier}`, async () => {
          const logging = await client.send(new DescribeLoggingStatusCommand({ 
      ...
      await this.safeExecute('snapshots', async () => {
        const snapshots = await client.send(new DescribeClusterSnapshotsCommand({ SnapshotType: 'manual' }));
      ...
      this.warn('Redshift scan failed', { error: (error as Error).message });
    }
...
export async function scanRedshift(
  region: string,
  accountId: string,
