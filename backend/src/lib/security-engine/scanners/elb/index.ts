...
export class ELBScanner extends BaseScanner {
  get serviceName(): string { return 'ELB'; }
  ...
  async scan(): Promise<Finding[]> {
    ...
    try {
      ...
      this.warn('Failed to list load balancers', { error: (error as Error).message });
    }
  ...
  private async checkLoadBalancer(client: ElasticLoadBalancingV2Client, lb: any): Promise<Finding[]> {
    const findings: Finding[] = [];
    ...
    try {
      ...
      this.warn(`Failed to get listeners for ${lbName}`, { error: (error as Error).message });
    }

    try {
      const attrs = await client.send(new DescribeLoadBalancerAttributesCommand({ LoadBalancerArn: lbArn }));
      const attributes = attrs.Attributes || [];
      
      const accessLogs = attributes.find((a: any) => a.Key === 'access_logs.s3.enabled');
      ...
      this.warn(`Failed to get attributes for ${lbName}`, { error: (error as Error).message });
    }
