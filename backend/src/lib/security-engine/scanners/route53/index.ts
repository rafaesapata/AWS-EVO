...
export class Route53Scanner extends BaseScanner {
  get serviceName(): string { return 'Route53'; }
  get category(): string { return 'DNS Security'; }
  ...
  async scan(): Promise<Finding[]> {
    ...
    try {
      ...
      this.warn('Route 53 scan failed', { error: (error as Error).message });
    }
  ...
  private async checkHostedZone(client: Route53Client, zone: any): Promise<Finding[]> {
    const findings: Finding[] = [];
    ...
    if (!zone.Config?.PrivateZone) {
      try {
        const dnssec = await client.send(new GetDNSSECCommand({ HostedZoneId: zoneId }));
    ...
    try {
      const logging = await client.send(new ListQueryLoggingConfigsCommand({ HostedZoneId: zoneId }));
  ...
  private async checkHealthChecks(client: Route53Client): Promise<Finding[]> {
    const findings: Finding[] = [];

    try {
      const healthChecks = await client.send(new ListHealthChecksCommand({}));
      ...
      this.warn('Failed to check health checks', { error: (e as Error).message });
    }
...
export async function scanRoute53(
  region: string,
  accountId: string,
