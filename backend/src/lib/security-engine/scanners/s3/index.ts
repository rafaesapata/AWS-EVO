...
export class S3Scanner extends BaseScanner {
  ...
  async scan(): Promise<Finding[]> {
    ...
    try {
      ...
      this.warn('Failed to list S3 buckets', { error: (error as Error).message });
    }
  ...
  private async scanBucket(client: S3Client, bucketName: string): Promise<Finding[]> {
    const findings: Finding[] = [];
    ...
    await this.safeExecute('logging', async () => {
      const response = await client.send(new GetBucketLoggingCommand({ Bucket: bucketName }));
    ...
    await this.safeExecute('bucketPolicy', async () => {
      try {
        const policyResponse = await client.send(new GetBucketPolicyCommand({ Bucket: bucketName }));
    ...
    await this.safeExecute('acl', async () => {
      const response = await client.send(new GetBucketAclCommand({ Bucket: bucketName }));
    ...
    await this.safeExecute('website', async () => {
      try {
        const response = await client.send(new GetBucketWebsiteCommand({ Bucket: bucketName }));
        ...
        if (e.name !== 'NoSuchWebsiteConfiguration') throw e;
      }
    }, null);
    ...
    await this.safeExecute('cors', async () => {
      try {
        const response = await client.send(new GetBucketCorsCommand({ Bucket: bucketName }));
