...
export class GlueScanner extends BaseScanner {
  ...
  private async checkJobEncryption(client: GlueClient): Promise<Finding[]> {
    const findings: Finding[] = [];
    ...
    try {
      ...
      this.warn('Failed to check Glue jobs', { error: (error as Error).message });
    }
    
    return findings;
  }

  private async checkDataCatalogEncryption(client: GlueClient): Promise<Finding[]> {
    const findings: Finding[] = [];
    
    try {
      const response = await client.send(new GetDataCatalogEncryptionSettingsCommand({}));
      ...
      this.warn('Failed to check Data Catalog encryption', { error: (error as Error).message });
    }
    
    return findings;
  }

  private async checkConnectionSSL(client: GlueClient): Promise<Finding[]> {
    const findings: Finding[] = [];
    
    try {
      const response = await client.send(new GetConnectionsCommand({}));
      ...
      this.warn('Failed to check Glue connections', { error: (error as Error).message });
    }
    
    return findings;
  }

  private async checkCrawlerPermissions(client: GlueClient): Promise<Finding[]> {
    const findings: Finding[] = [];
    
    try {
      const response = await client.send(new GetCrawlersCommand({}));
      ...
      this.warn('Failed to check Glue crawlers', { error: (error as Error).message });
    }
    
    return findings;
  }

  private async checkSecurityConfigurations(client: GlueClient): Promise<Finding[]> {
    const findings: Finding[] = [];
    
    try {
      const response = await client.send(new GetSecurityConfigurationsCommand({}));
      ...
      if (!response.SecurityConfigurations || response.SecurityConfigurations.length === 0) {
        ...
        for (const config of response.SecurityConfigurations) {
          if (!config.Name || !config.EncryptionConfiguration) continue;
      ...
      this.warn('Failed to check Glue security configurations', { error: (error as Error).message });
    }
    
    return findings;
  }

  private async checkDatabaseSecurity(client: GlueClient): Promise<Finding[]> {
    const findings: Finding[] = [];
    
    try {
      const response = await client.send(new GetDatabasesCommand({}));
      for (const database of response.DatabaseList || []) {
        ...
        try {
          const tablesResponse = await client.send(new GetTablesCommand({ DatabaseName: database.Name }));
      ...
      this.warn('Failed to check Glue databases', { error: (error as Error).message });
    }
...
export async function scanGlue(
  region: string,
  accountId: string,
