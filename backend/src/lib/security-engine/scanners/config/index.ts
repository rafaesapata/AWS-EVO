...
export class ConfigScanner extends BaseScanner {
  get serviceName(): string { return 'Config'; }
  ...
  async scan(): Promise<Finding[]> {
    ...
    try {
      ...
      await this.safeExecute('conformance-packs', async () => {
        const conformancePacks = await client.send(new DescribeConformancePacksCommand({}));
      ...
      await this.safeExecute('compliance-summary', async () => {
        const compliance = await client.send(new GetComplianceSummaryByConfigRuleCommand({}));
      ...
      this.warn('Config scan failed', { error: (error as Error).message });
    }
