/**
 * Security Engine V2 - ElastiCache Scanner
 */
...
export class ElastiCacheScanner extends BaseScanner {
  get serviceName(): string { return 'ElastiCache'; }
  ...
  private async checkClusters(client: ElastiCacheClient): Promise<Finding[]> {
    const findings: Finding[] = [];
    ...
    try {
      ...
      this.warn('Failed to describe clusters', { error: (error as Error).message });
    }

    return findings;
  }

  private async checkReplicationGroups(client: ElastiCacheClient): Promise<Finding[]> {
    const findings: Finding[] = [];
    ...
    try {
      ...
      this.warn('Failed to describe replication groups', { error: (error as Error).message });
    }
...
export async function scanElastiCache(
  region: string,
  accountId: string,
