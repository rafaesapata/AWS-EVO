...
export class InspectorScanner extends BaseScanner {
  get serviceName(): string { return 'Inspector'; }
  get category(): string { return 'Security'; }
  ...
  async scan(): Promise<Finding[]> {
    ...
    try {
      ...
      const accountStatus = await client.send(new BatchGetAccountStatusCommand({
      ...
      await this.safeExecute('critical-findings', async () => {
        const criticalFindings = await client.send(new ListFindingsCommand({
          filterCriteria: {
      ...
      this.warn('Inspector scan failed', { error: (error as Error).message });
    }
...
export async function scanInspector(
  region: string,
  accountId: string,
