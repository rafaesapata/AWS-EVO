 ...
 */
export function toLegacyResult(result: MLResult): MLResultLegacy {
  return {
 ...
 */
export function calculatePriority(monthlySavings: number, confidence: number): number {
  // Priority 5: > $500/month with high confidence
  if (monthlySavings > 500 && confidence > 0.8) return 5;
  // Priority 4: > $200/month or > $500 with medium confidence
  if (monthlySavings > 200 || (monthlySavings > 500 && confidence > 0.6)) return 4;
  // Priority 3: > $50/month
  if (monthlySavings > 50) return 3;
  // Priority 2: > $10/month
  if (monthlySavings > 10) return 2;
