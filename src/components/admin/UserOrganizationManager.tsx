...
export default function UserOrganizationManager() {
  ...
  const checkSuperAdminStatus = async () => {
    ...
    const response = await apiClient.select('profiles', {
      eq: { id: user.id, role: 'super_admin' }
    });
    setIsSuperAdmin(!!(response.data && response.data.length > 0));
  };

  const loadData = async () => {
    const profilesResponse = await apiClient.select<Profile>('profiles', {
      order: { column: 'full_name', ascending: true }
    });
    const profilesData = profilesResponse.data || [];
    setProfiles(profilesData);

    const orgsResponse = await apiClient.select<Organization>('organizations', {
      order: { column: 'name', ascending: true }
    });
    const orgsData = orgsResponse.data || [];
    setOrganizations(orgsData);

    const userOrgsResponse = await apiClient.select('user_organizations', {
      order: { column: 'created_at', ascending: false }
    });
    const userOrgsRaw = userOrgsResponse.data || [];
    ...
    const userOrgs: UserOrganization[] = userOrgsRaw
      .map((uo: any) => {
        ...
        return { ...uo, profiles: profile, organizations: org };
      })
      .filter((uo: any): uo is UserOrganization => uo !== null);
  ...
  const addUserToOrganization = async () => {
    if (!selectedUser || !selectedOrg) {
      toast({ variant: "destructive", title: t('common.error'), description: "Please select both user and organization" });
      return;
    }

    setIsLoading(true);
    const response = await apiClient.rpc('add_user_to_organization', {
    ...
    if (response.error) {
      toast({ variant: "destructive", title: t('common.error'), description: response.error.message });
    } else {
      toast({ title: t('common.success'), description: "User added to organization successfully" });
      setSelectedUser("");
      setSelectedOrg("");
      setIsPrimary(false);
      loadData();
  ...
  const removeUserFromOrganization = async (userId: string, orgId: string) => {
    ...
    const response = await apiClient.rpc('remove_user_from_organization', {
      _user_id: userId,
    ...
    if (response.error) {
      ...
      toast({ title: t('common.success'), description: "User removed from organization successfully" });
      loadData();
    }
  };
  ...
  return (
    <Card>
      <CardHeader>
        ...
        <CardDescription>Manage which users have access to which organizations</CardDescription>
      </CardHeader>
      <CardContent className="space-y-6">
        <div className="space-y-4 p-4 border rounded-lg bg-muted/50">
          ...
          <Button onClick={addUserToOrganization} disabled={isLoading || !selectedUser || !selectedOrg} className="w-full">
            <Plus className="h-4 w-4 mr-2" />Add User to Organization
          </Button>
        </div>

        <div className="relative">
          <Search className="absolute left-3 top-1/2 transform -translate-y-1/2 h-4 w-4 text-muted-foreground" />
          <Input placeholder="Search users or organizations..." value={searchTerm} onChange={(e) => setSearchTerm(e.target.value)} className="pl-9" />
        </div>
          ...
          <Table>
            ...
            <TableBody>
              {filteredUserOrgs.length === 0 ? (
                <TableRow><TableCell colSpan={5} className="text-center text-muted-foreground">No user organizations found</TableCell></TableRow>
              ) : (
                filteredUserOrgs.map((uo) => (
                  <TableRow key={uo.id}>
                    ...
                    <TableCell>{uo.is_primary && <Badge variant="default">Primary</Badge>}</TableCell>
                    <TableCell className="text-right">
                      <Button variant="ghost" size="sm" onClick={() => removeUserFromOrganization(uo.user_id, uo.organization_id)}>
