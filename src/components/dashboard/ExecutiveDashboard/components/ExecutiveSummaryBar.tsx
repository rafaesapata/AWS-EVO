      ...
      <Card className={cn('border card-hover-lift card-shine', getScoreBg(data.overallScore))}>
        <CardContent className="p-4">
          <div className="flex items-center justify-between mb-2">
            ...
            <Shield className={cn('h-5 w-5 icon-pulse', getScoreColor(data.overallScore))} />
          </div>
          <div className="flex items-baseline gap-2">
            <span className={cn('text-3xl font-bold tabular-nums', getScoreColor(data.overallScore))}>
              {data.overallScore}
            </span>
            <span className="text-muted-foreground">/100</span>
            {data.scoreChange !== 0 && (
              <Badge variant={data.scoreChange > 0 ? 'default' : 'destructive'} className="ml-2">
                {data.scoreChange > 0 ? <TrendingUp className="h-3 w-3 mr-1" /> : <TrendingDown className="h-3 w-3 mr-1" />}
      ...
      <Card className="card-hover-lift card-shine">
        <CardContent className="p-4">
          <div className="flex items-center justify-between mb-2">
            ...
            <DollarSign className="h-5 w-5 text-muted-foreground icon-bounce" />
          </div>
          ...
          <div className="space-y-1">
            ...
            <Progress value={budgetPercentage} className={cn('h-1.5', getBudgetColor())} />
          </div>
        </CardContent>
      </Card>
      ...
      <Card className="bg-green-500/5 border-green-500/20 card-hover-lift card-shine glow-success">
        <CardContent className="p-4">
          <div className="flex items-center justify-between mb-2">
            ...
            <TrendingUp className="h-5 w-5 text-green-500 icon-bounce" />
          </div>
      ...
      <Card className="card-hover-lift card-shine">
        <CardContent className="p-4">
          <div className="flex items-center justify-between mb-2">
            ...
            <Activity className="h-5 w-5 text-muted-foreground icon-pulse" />
          </div>
          <div className={cn(
          ...
          <span className="text-xs text-muted-foreground">Target: 99.9%</span>
        </CardContent>
      </Card>
          ...
          <div className="flex items-baseline gap-3">
            {data.activeAlerts.critical > 0 && (
              <div className="flex items-center gap-1">
                <span className="text-2xl font-bold text-red-500 tabular-nums">{data.activeAlerts.critical}</span>
                <span className="text-xs text-red-500">CRIT</span>
              </div>
            )}
            {data.activeAlerts.high > 0 && (
              <div className="flex items-center gap-1">
                <span className="text-xl font-bold text-orange-500 tabular-nums">{data.activeAlerts.high}</span>
                <span className="text-xs text-orange-500">HIGH</span>
              </div>
            )}
            {data.activeAlerts.medium > 0 && (
              <div className="flex items-center gap-1">
                <span className="text-lg font-semibold text-yellow-500 tabular-nums">{data.activeAlerts.medium}</span>
                <span className="text-xs text-yellow-500">MED</span>
              </div>
            )}
            {totalAlerts === 0 && (
              <span className="text-2xl font-bold text-green-500 tabular-nums">0</span>
            )}
          </div>
        </CardContent>
      </Card>
