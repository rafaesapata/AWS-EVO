...
export default function SecurityAnalysisContent() {
 ...
 <div className={`text-6xl font-semibold mb-2 tabular-nums animate-in fade-in-0 zoom-in-95 duration-500 ${getScoreColor(posture?.overall_score || 0)}`}>
 ...
 <div className={`text-center p-4 bg-red-500/10 rounded-lg border border-red-500/20 transition-all hover:scale-105 ${criticalCount > 0 ? ' ' : ''}`}>
 <div className="text-3xl font-semibold text-red-500 tabular-nums">{criticalCount}</div>
 ...
 <div className="text-3xl font-semibold text-orange-500 tabular-nums">{highCount}</div>
 ...
 <div className="text-3xl font-semibold text-yellow-500 tabular-nums">{mediumCount}</div>
 <div className="text-sm text-muted-foreground">Médios</div>
 </div>
 <div className="text-center p-4 bg-blue-500/10 rounded-lg border border-blue-500/20 transition-all hover:scale-105 ">
 <div className="text-3xl font-semibold text-blue-500 tabular-nums">{lowCount}</div>
 ...
 <div className="flex items-center gap-2">
 <span className={`font-semibold ${category.total > 0 ? (category.critical > 0 ? 'text-red-500' : category.high > 0 ? 'text-orange-500' : 'text-yellow-500') : 'text-green-500'}`}>
 {category.total}
 </span>
 {category.total > 0 ? (
 ...
 <Progress value={category.score} className="" />
 </div>
 ))}
 </CardContent>
 </Card>
