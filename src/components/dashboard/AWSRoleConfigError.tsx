...
import { AlertTriangle, Copy, ExternalLink, Shield, RefreshCw } from "lucide-react";
import { Button } from "@/components/ui/button";
import { toast } from "sonner";
import { useTranslation } from "react-i18next";
import { useNavigate } from "react-router-dom";

interface AWSRoleConfigErrorProps {
  errorMessage: string;
  accountName?: string;
  externalId?: string;
}

export default function AWSRoleConfigError({ 
  errorMessage,
  accountName,
  externalId
}: AWSRoleConfigErrorProps) {
      ...
      <CardContent className="space-y-4">
        ...
        <div className="bg-blue-500/10 border border-blue-500/20 rounded-lg p-4">
          ...
          <ol className="text-sm space-y-3 list-decimal list-inside text-muted-foreground">
            <li>
              {t('aws.step1AccessIAM', 'Acesse o AWS IAM Console na conta')} <strong>{targetAccountId}</strong>
            </li>
            <li>
              {t('aws.step2FindRole', 'Localize a role')} <strong>{roleName}</strong>
            </li>
            <li>
              {t('aws.step3ClickTrustRelationships', 'Clique na aba "Trust relationships" (Relações de confiança)')}
            </li>
            <li>
              {t('aws.step4EditTrustPolicy', 'Clique em "Edit trust policy" e substitua pelo JSON abaixo')}
            </li>
            <li>
              {t('aws.step5ReplaceExternalId', 'Substitua')} <code className="bg-muted px-1 py-0.5 rounded">SEU_EXTERNAL_ID_AQUI</code> {t('aws.step5ByExternalId', 'pelo External ID da sua conta (visível nas configurações AWS)')}
            </li>
            <li>
              {t('aws.step6Save', 'Salve a política e tente novamente')}
            </li>
          </ol>
        </div>
        ...
        <div>
          <div className="flex items-center justify-between mb-2">
            <div className="text-sm font-semibold">
              {t('aws.trustPolicyJson', 'Trust Policy JSON')}
            </div>
            <Button 
              variant="outline" 
              size="sm"
        ...
        <div className="bg-orange-500/10 border border-orange-500/20 rounded-lg p-4">
          <div className="flex items-start gap-2">
            ...
            <div className="text-sm text-orange-800 dark:text-orange-300">
              <strong>{t('aws.important', 'Importante')}:</strong> {t('aws.externalIdNote', 'O External ID é único para cada conta AWS conectada e pode ser encontrado nas Configurações AWS desta plataforma. Ele é necessário para segurança e deve corresponder exatamente.')}
            </div>
          </div>
        </div>
