...
export const WellArchitectedHistory = ({ organizationId, onViewScan }: WellArchitectedHistoryProps) => {
 ...
 className={selectedPeriod !== period ? 'bg-gray-50' : ''}
 >
 {period === '7d' && '7 dias'}
 {period === '30d' && '30 dias'}
 {period === '90d' && '90 dias'}
 ...
 <p className="text-2xl font-semibold tabular-nums">{latestScan.overall_score.toFixed(1)}</p>
 </div>
 ...
 <Card className={` ${latestScan.critical_issues > 0 ? '' : ''}`}>
 <CardContent className="pt-6">
 <div className="flex items-center justify-between">
 <div>
 <p className="text-sm text-muted-foreground">Issues Críticos</p>
 <p className="text-2xl font-semibold tabular-nums">{latestScan.critical_issues}</p>
 </div>
 ...
 <Card key={scan.id} className=" hover:bg-accent/50 transition-all hover:translate-x-1 ">
 ...
 <Badge variant="outline" className={`bg-green-500/10 text-green-500 border-green-500/20 ${scan.overall_score >= 80 ? '' : ''}`}>
 Score: <span className="tabular-nums">{scan.overall_score.toFixed(1)}</span>
 </Badge>
 <Badge variant="outline" className={`bg-red-500/10 text-red-500 border-red-500/20 ${scan.critical_issues > 0 ? '' : ''}`}>
