...
export const SecurityAnalysisHistory = ({ organizationId, accountId, onViewScan }: SecurityAnalysisHistoryProps) => {
 ...
 <p className="text-2xl font-semibold tabular-nums">{latestScan.total_findings}</p>
 </div>
 ...
 <Card className={` ${latestScan.critical_count > 0 ? '' : ''}`}>
 <CardContent className="pt-6">
 <div className="flex items-center justify-between">
 <div>
 <p className="text-sm text-muted-foreground">Vulnerabilidades Críticas</p>
 <p className="text-2xl font-semibold tabular-nums">{latestScan.critical_count}</p>
 </div>
 ...
 <Badge variant="outline" className={`bg-red-500/10 text-red-500 border-red-500/20 ${scan.critical_count > 0 ? '' : ''}`}>
