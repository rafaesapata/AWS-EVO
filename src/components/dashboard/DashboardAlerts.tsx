...
export default function DashboardAlerts({ organizationId }: { organizationId: string }) {
  ...
  const { data: alerts, refetch } = useQuery({
    ...
    queryFn: async () => {
      const response = await apiClient.select<DashboardAlert>('dashboard_alerts', {
        eq: { organization_id: organizationId, is_read: false },
  ...
  const markAsRead = async (alertId: string) => {
    await apiClient.update('dashboard_alerts', { is_read: true }, { id: alertId });
    refetch();
  };
  ...
  return (
    <div className="space-y-3">
      {alerts.map(alert => (
        <Alert key={alert.id} className={cn("relative animate-fade-in", getSeverityClass(alert.severity))}>
          {!isTVMode && (
            <Button
              size="icon"
              variant="ghost"
          ...
          <div className="flex items-start gap-3 pr-8">
            ...
            <div className="flex-1">
              ...
              {alert.action_url && (
                <Button size="sm" variant="link" className="mt-2 p-0 h-auto" onClick={() => window.location.href = alert.action_url!}>
                  Tomar ação →
                </Button>
              )}
            </div>
