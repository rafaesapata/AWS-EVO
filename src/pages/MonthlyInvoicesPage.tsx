      ...
      {!isLoadingCosts && selectedMonthData && (
        <div className="grid gap-4 md:grid-cols-2 lg:grid-cols-4">
          <Card>
            ...
            <CardContent>
              <div className="text-2xl font-semibold"><CurrencyIndicator className="mr-1" />{sym}{convert(selectedMonthData.totalCost).toFixed(2)}</div>
          ...
          <Card>
            ...
            <CardContent>
              <div className="text-2xl font-semibold text-success">
                {sym}{convert(selectedMonthData.totalCredits).toFixed(2)}
              </div>
          ...
          <Card>
            ...
            <CardContent>
              <div className="text-2xl font-semibold">{sym}{convert(selectedMonthData.netCost).toFixed(2)}</div>
          ...
          <Card>
            ...
            <CardContent>
              <div className="text-2xl font-semibold">
                {sym}{convert(selectedMonthData.netCost / selectedMonthData.days).toFixed(2)}
              </div>
              <Button
                variant="outline"
                size="sm"
              ...
              <ResponsiveContainer width="100%" height={400}>
                <BarChart data={monthComparisonData}>
                  ...
                  <Tooltip 
                    ...
                    formatter={(value: number) => `${sym}${convert(value).toFixed(2)}`}
                  />
                  <Legend />
                  ...
                  <CardContent>
                    <ResponsiveContainer width="100%" height={300}>
                      <PieChart>
                        ...
                        <Tooltip formatter={(value: number) => `${sym}${convert(value).toFixed(2)}`} />
                      </PieChart>
                    </ResponsiveContainer>
                  </CardContent>
                </Card>
                ...
                >
                  ...
                  <div className="flex items-center gap-4">
                    ...
                    <div className="text-right">
                      <div className="font-mono font-semibold">{sym}{convert(data.netCost).toFixed(2)}</div>
                      {data.totalCredits > 0 && (
                        <div className="text-sm text-success font-mono">-{sym}{convert(data.totalCredits).toFixed(2)}</div>
                      )}
                    </div>
                    <Button
                      variant="outline"
