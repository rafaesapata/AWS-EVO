...
import { Tv, Plus, Copy, Trash2, ExternalLink, Clock, Calendar, RefreshCw, LayoutDashboard, Key } from "lucide-react";
import { format } from "date-fns";
import TVDashboardBuilder from "@/components/tv/TVDashboardBuilder";
...
export default function TVDashboardManagement() {
  ...
  const { data: tokens, isLoading } = useQuery({
    ...
    queryFn: async () => {
      ...
      if (result.error) throw new Error('Failed to load tokens');
  ...
  const createMutation = useMutation({
    mutationFn: async (data: { name: string; expirationDays: number }) => {
      ...
      if (result.error || !responseData?.success) throw new Error(responseData?.error || 'Failed to create token');
      return result.data;
    },
    onSuccess: () => {
  ...
  const toggleMutation = useMutation({
    mutationFn: async (data: { tokenId: string; isActive: boolean }) => {
      ...
      const responseData = result.data as { success?: boolean; error?: string };
      if (result.error || !responseData?.success) throw new Error(responseData?.error || 'Failed to update token');
      return result.data;
    },
    onSuccess: () => {
      queryClient.invalidateQueries({ queryKey: ['tv-tokens'] });
  ...
  const deleteMutation = useMutation({
    mutationFn: async (tokenId: string) => {
      ...
      const responseData = result.data as { success?: boolean; error?: string };
      if (result.error || !responseData?.success) throw new Error(responseData?.error || 'Failed to delete token');
      return result.data;
    },
    onSuccess: () => {
      queryClient.invalidateQueries({ queryKey: ['tv-tokens'] });
  ...
  const copyToClipboard = (token: string) => {
    ...
    toast({ title: t('tvDashboard.linkCopied') });
  };

  const openInNewTab = (token: string) => window.open(`/tv/${token}`, '_blank');
    ...
    >
      <Tabs defaultValue="dashboards" className="space-y-6">
        <TabsList className="glass">
          <TabsTrigger value="dashboards"><LayoutDashboard className="h-4 w-4 mr-2" />{t('tvDashboard.createDashboard', 'TV Dashboards')}</TabsTrigger>
          <TabsTrigger value="tokens"><Key className="h-4 w-4 mr-2" />{t('tvDashboard.createToken', 'Tokens')}</TabsTrigger>
        </TabsList>
