...
- [x] 1. Corrigir domínios hardcoded no buildspec e SAM template
  - [x] 1.1 Corrigir `cicd/buildspec-sam.yml` — bloco sandbox: alterar APP_DOMAIN para `evo.sandbox.nuevacore.com`, API_DOMAIN para `api.evo.sandbox.nuevacore.com`, AZURE_OAUTH_REDIRECT_URI para `https://evo.sandbox.nuevacore.com/azure/callback`
  ...
  - [x] 1.2 Corrigir `sam/production-lambdas-only.yaml` — remover defaults hardcoded de AppDomain, ApiDomain e AzureOAuthRedirectUri (deixar Default vazio '')
    - Também corrigir em `sam/template.yaml` se necessário
    - _Requirements: 9.5_
  - [x] 1.3 Atualizar `.env.development` — alterar VITE_API_BASE_URL para `https://api.evo.sandbox.nuevacore.com`
    - _Requirements: 9.3_

- [x] 2. Criar script de configuração de SSM Parameters
  - [x] 2.1 Criar `scripts/setup-sandbox-ssm.sh` que configura todos os parâmetros SSM necessários para o sandbox
...
- [x] 3. Criar script de dump/restore do banco de dados
  - [x] 3.1 Criar `scripts/sandbox-db-restore.sh` que faz dump do banco de produção e restore no sandbox
...
- [x] 4. Checkpoint — Verificar correções de código
...
- [x] 5. Criar script de setup do API Gateway Custom Domain
  - [x] 5.1 Criar `scripts/setup-sandbox-api-domain.sh` que configura o custom domain no API Gateway do sandbox
...
- [x] 6. Criar script de setup do CloudFront Custom Domain
  - [x] 6.1 Criar `scripts/setup-sandbox-cloudfront.sh` que configura o alias no CloudFront do sandbox
...
- [x] 7. Criar script de deploy do CI/CD Pipeline
  - [x] 7.1 Criar `scripts/setup-sandbox-pipeline.sh` que deploya o CodePipeline para sandbox
...
- [x] 8. Criar script de verificação do ambiente
  - [x] 8.1 Criar `scripts/verify-sandbox.sh` que valida o ambiente completo
...
- [x] 9. Atualizar documentação steering
  - [x] 9.1 Atualizar `.kiro/steering/infrastructure.md` com informações completas do sandbox
...
- [x] 10. Criar script master de setup completo
  - [x] 10.1 Criar `scripts/setup-sandbox-complete.sh` que orquestra todos os scripts na ordem correta
...
- [x] 11. Final checkpoint — Verificação completa
  - Ensure all tests pass, ask the user if questions arise.
